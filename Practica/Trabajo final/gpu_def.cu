...
#ifdef WIN
#include <time.h>
#define DECLARACION clock_t start,finish;double duration;
#define START start = clock();
#define STOP finish = clock();(double)(finish - start) / CLOCKS_PER_SEC;
#else
#include <sys/time.h>
#include <sys/resource.h>
#define DECLARACION double timetick;
#define START timetick = dwalltime();
#define STOP dwalltime() - timetick
#endif
...
int main(int argc, char const *argv[])
{
        ...
        // Grid unidimensional (*ceil(n/blk_size)* bloques)
        //printf("%d,,%d,,%d,, \n",limite / thr_x_blk;, limite, thr_x_blk);    
