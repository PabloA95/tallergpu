...
__global__ void cuadradoV_kernel_cuda(int *const arrayV,   const int n,   const int c){
...
void cuadradoV_GPU( int arrayV[], const int n, const int blk_size,   const int c){
double timetick;
  ...
  timetick = dwalltime();
  cuadradoV_kernel_cuda<<<dimGrid, dimBlock>>>(cV, n, c);
